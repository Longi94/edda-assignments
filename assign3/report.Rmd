...
We can distinguish extreme values outstanding from trend line, normality is doubtful. We should transform the data or consider outliers.
...
We test the null hypothesis with a 1-way ANOVA test.
...
We get a p-value of **0.967**5, which means that we cannot reject the null hypothesis.

**Point 2**

We estimate the differnce using a paired two sample t-test.

```{r}
t.test(milk[treatment == "A"], milk[treatment == "B"], paired = TRUE)
```

The resulting mean of differences is **0.244**.
...
Here we get a p-value of **0.446** so the null hypthesis cannot be rejected again. This is the same result as the one we got int Point 1 but with a different p-value.

TODO EXPLAIN WHY?

**Point 4**

This is the same piece of code we used in Point 2 to calculate the estimate difference between milk productions. The test returned a p-value of **0.8281**, which is compatible with the other results in that we also cannot reject the null hypothesis.
...
According to these plots we cannot assume normality and there are extreme values as well. The spread of the residuals is smaller for small values and lot higher for high values.
